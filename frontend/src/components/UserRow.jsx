...
export default function UserRow({
  ...
  const date = Number.isNaN(parsedDate.getTime()) ? 'Recently' : new Intl.DateTimeFormat('en-US', {
    month: 'short',
    day: '2-digit',
    year: 'numeric'
  }).format(parsedDate);
  return <div className="grid gap-4 border-b border-slate-200 py-4 last:border-b-0 md:grid-cols-[1.4fr_1fr_0.7fr_0.9fr_auto] md:items-center">
      <div className="flex items-center gap-3">
        <Avatar label={`${user.displayName} avatar`} role={user.role} />
        <div>
          <p className="font-semibold text-slate-900">
{user.displayName}
</p>
          <p className="text-sm text-slate-500">@{user.username}
</p>
        </div>
      </div>
      <span className={`w-fit rounded-full px-3 py-0.5 text-sm font-medium ring-1 ${badgeClass}`}>
{user.role === 'admin' ? 'Admin' : 'user'}
</span>
      <time className="text-sm text-slate-500">
{date}
</time>
      <span className="text-sm text-slate-500 md:hidden">Created {date}
</span>
      <div>
        {isDefaultAdmin ? (
          <span
          ...
          <button
            ...
            title={canDelete ? 'Delete user' : 'You cannot delete the current account'}
            type="button"
          >
            Delete
          </button>
