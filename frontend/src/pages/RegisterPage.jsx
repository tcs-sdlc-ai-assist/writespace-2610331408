...
export default function RegisterPage() {
  ...
  function handleSubmit(event) {
    ...
    if (!setSession(session)) {
      setErrors({
        form: 'Your browser could not start a session.'
      });
      return;
    }
    navigate('/blogs');
  ...
  return <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-indigo-600 via-violet-600 to-pink-500 px-4 py-10">
...
<p className="mt-8 text-sm font-bold uppercase tracking-[0.2em] text-pink-500">Start locally</p>
<h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-900" id="register-title">Create your writing space.</h1>
<form className="mt-8 space-y-4" noValidate onSubmit={handleSubmit}>
{formFields.map(([name, label, type]) => <div key={name}>
...
aria-required="true"
className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-slate-900 outline-none transition-shadow focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
id={name}
name={name}
...
</div>)}{errors.form && <p className="rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-600" role="alert">
{errors.form}
</p>}
<button
className="w-full rounded-lg bg-indigo-600 px-4 py-2.5 font-medium text-white shadow-sm transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
type="submit"
>
Create Account
</button>
